...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:ns0="http://hl7.org/fhir" xmlns:provisio="http://www.altova.com/MapForce/UDF/provisio" xmlns:tbf="http://www.altova.com/MapForce/UDF/tbf" xmlns:vmf="http://www.altova.com/MapForce/UDF/vmf" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:fn="http://www.w3.org/2005/xpath-functions" xmlns:xml="http://www.w3.org/XML/1998/namespace" xmlns:ns1="urn:cz-mzcr:ns:dasta:ds4:ds_dasta" xmlns:ns2="urn:cz-mzcr:ns:dasta:ds4:ds_ip" exclude-result-prefixes="ns0 provisio tbf vmf xs fn ns1 ns2">
	...
	<xsl:template name="provisio:ku_o_lab_generalAttribute">
		...
		<xsl:variable name="var22_lop" as="node()*" select="$ku_o_labType/ns2:lop"/>
		<xsl:variable name="var23_lopk" as="node()*" select="$ku_o_labType/ns2:lopk"/>
		<xsl:variable name="var24_http___loinc_org_as_string" as="xs:string" select="xs:string(xs:anyURI('http://loinc.org'))"/>
		<xsl:variable name="var25_lopr" as="node()*" select="$ku_o_labType/ns2:lopr"/>
		<xsl:variable name="var26_lopz" as="node()*" select="$ku_o_labType/ns2:lopz"/>
		<xsl:variable name="var27_loi" as="node()*" select="$ku_o_labType/ns2:loi"/>
				...
				<Composition>
					...
					<section>
						<code>
							<coding>
								<system>
									<xsl:attribute name="value" namespace="" select="$var24_http___loinc_org_as_string"/>
								</system>
								<code>
					...
					<section>
						...
						<xsl:for-each select="$var27_loi/@guid">
							<entry>
								<reference>
					...
					<section>
						...
						<xsl:for-each select="$var22_lop/@guid">
							<entry>
								<reference>
						...
						<xsl:for-each select="$var26_lopz/@guid">
							<entry>
								<reference>
									<xsl:attribute name="value" namespace="" select="fn:concat('ServiceRequest/', fn:string(.))"/>
								</reference>
		...
		<xsl:for-each select="$var26_lopz">
	...
	<xsl:template name="vmf:vmf1_inputtoresult">
		...
		<xsl:choose>
			<xsl:when test="$input='R'">
				<xsl:copy-of select="'číslo'"/>
			</xsl:when>
			<xsl:when test="$input='M'">
				<xsl:copy-of select="'menší než'"/>
			</xsl:when>
			<xsl:when test="$input='V'">
				<xsl:copy-of select="'větší než'"/>
			</xsl:when>
		</xsl:choose>
	</xsl:template>
	<xsl:template name="vmf:vmf2_inputtoresult">
		...
		<xsl:choose>
			<xsl:when test="$input='Z'">
				<xsl:copy-of select="'začátek'"/>
			</xsl:when>
			<xsl:when test="$input='P'">
				<xsl:copy-of select="'průběh'"/>
			</xsl:when>
	...
	<xsl:template name="vmf:vmf3_inputtoresult">
		...
		<xsl:choose>
			<xsl:when test="$input='V'">
				<xsl:copy-of select="'Vitální indikace'"/>
			</xsl:when>
			<xsl:when test="$input='S'">
				<xsl:copy-of select="'Vše statim'"/>
			</xsl:when>
			<xsl:when test="$input='P'">
				<xsl:copy-of select="'Vše přednostně'"/>
			</xsl:when>
			<xsl:when test="$input='R'">
				<xsl:copy-of select="'Vše rutina'"/>
			</xsl:when>
			<xsl:when test="$input='K'">
